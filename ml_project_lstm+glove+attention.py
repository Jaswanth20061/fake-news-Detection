# -*- coding: utf-8 -*-
"""ML-project-LSTM+Glove+Attention.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1LcngD8V78yfNJmjja896I2TpC5DmYmaZ
"""
...
def word_to_matrix(glove_vec, token, nb_words):
  embedding_matrix = np.zeros((nb_words, EMBEDDING_DIM))
  for word, index in token.word_index.items():
    if(index> MAX_NB_WORDS):
        continue
    embedding_vector = glove_vec.get(word) 
...
X_train = pad_sequences(encoded_text, maxlen=MAX_SEQUENCE_LENGTH, padding='post')
print(len(X_train))
print(X_train[0])
...
saved = True
filename = '/content/drive/My Drive/ML/ML_Assignment_6/'
if saved == True:
  glove_vec = joblib.load(filename+'glove')
else:
  glove_vec = read_glove_vecs(filename+'glove.6B.50d.txt')
  joblib.dump(glove_vec, filename+'glove')

print(len(glove_vec))
embedding_matrix = word_to_matrix(glove_vec, token, nb_words)
...
def set_model(embedding_vectors):
  ...
  merged = Dense(num_lstm, activation = act)(merged)
